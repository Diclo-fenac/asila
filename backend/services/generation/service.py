...
async def _translate_query(query: str):
    ...
    return lang_code, translated_query

async def _retrieve_and_fuse(db: AsyncSession, query_vector: list):
    """Run 5 parallel ANN searches and fuse results via RRF."""
    ...
    chunk_cands = [
        {"id": str(r[0]), "type": "chunk", "content": r[1], "document_id": str(r[2]), "page_number": r[3], "document_title": r[4]}
        for r in chunk_res.all()
    ]
    ent_cands = [
        {"id": str(r[0]), "type": "entity", "content": f"{r[1]} - {r[2]}", "document_id": str(r[0]), "page_number": None, "document_title": f"Entity: {r[1]}"}
        for r in ent_res.all()
    ]
    rel_cands = [
        {"id": str(r[0]), "type": "relationship", "content": r[1], "document_id": str(r[0]), "page_number": None, "document_title": f"Relationship: {r[1]}"}
        for r in rel_res.all()
    ]
    comm_cands = [
        {"id": str(r[0]), "type": "community", "content": f"{r[1]} - {r[2]}", "document_id": str(r[0]), "page_number": None, "document_title": f"Community: {r[1]}"}
        for r in comm_res.all()
    ]
    sum_cands = [
        {"id": str(r[0]), "type": "summary", "content": r[1], "document_id": str(r[2]), "page_number": None, "document_title": f"Summary: {r[3]}"}
        for r in sum_res.all()
    ]

    return reciprocal_rank_fusion([chunk_cands, ent_cands, rel_cands, comm_cands, sum_cands])

async def _build_context(db: AsyncSession, fused: list):
    """Build context text and citations from fused results with graph expansion."""
...
async def answer_query(db: AsyncSession, query: str, user_id: Optional[str] = None, conversation_id: Optional[str] = None) -> AnswerResponse:
    lang_code, translated_query = await _translate_query(query)
    query_vector = await gemini_service.generate_embedding(translated_query)
    fused = await _retrieve_and_fuse(db, query_vector)
    ...
    context_text, citations = await _build_context(db, fused)
