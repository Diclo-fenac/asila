...
@router.get("/")
async def list_documents(db: AsyncSession = Depends(get_tenant_db)):
...
@router.get("/{id}")
async def get_document(id: str, db: AsyncSession = Depends(get_tenant_db)):
...
@router.delete("/{id}")
async def delete_document(id: str, db: AsyncSession = Depends(get_tenant_db)):
    result = await db.execute(select(Document).where(Document.id == id))
    doc = result.scalar_one_or_none()
    if not doc:
        raise HTTPException(status_code=404)
